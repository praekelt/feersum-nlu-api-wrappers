...
# **data_object_untrash**
> DataObject data_object_untrash(instance_name, x_caller=x_caller, origin=origin)
...
try:
    # Untrash the named data_object.
    api_response = api_instance.data_object_untrash(instance_name, x_caller=x_caller, origin=origin)
    pprint(api_response)
except ApiException as e:
